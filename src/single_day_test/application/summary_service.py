...
def _build(context: RunContext, state: RuntimeState, ended: datetime, status: RunStatus, error: Exception | None) -> RunSummary:
    c=state.channel
    return RunSummary(context.run_id,context.symbol,context.trade_date,context.mode,context.direction,context.parameter_set.parameter_set_id,asdict(context.parameter_set),context.initial_threshold,state.processed_bar_count,len(state.signal_events),c.curr_trend_slope,c.curr_trend_intercept,c.curr_high_percentile,c.curr_low_percentile,len(c.bars),status,context.started_at_et,ended,type(error).__name__ if error else None,str(error) if error else None)
