...
def test_load_parameter_set_by_id(tmp_path) -> None:
    ...
    path.write_text("parameter_set_id,trend_window,channel_window,r2_threshold,channel_high_percentile,channel_low_percentile,continuous_break_count,is_active\nalpha,20,10,0.5,95,95,2,0\n", encoding="utf-8")
...
def test_active_selection_and_explicit_inactive_override(tmp_path) -> None:
    path = tmp_path / "parameter_set.csv"
    path.write_text(
        "parameter_set_id,trend_window,channel_window,r2_threshold,channel_high_percentile,channel_low_percentile,continuous_break_count,is_active\n"
        "active,20,10,0.5,95,95,2,1\n"
        "inactive,20,10,0.5,95,95,2,0\n",
        encoding="utf-8",
    )
