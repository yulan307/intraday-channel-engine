...
def test_live_runner_processes_hist_live_end_and_writes_jsonl(tmp_path: Path) -> None:
    ...
    daily_statistics = database.connection.execute("SELECT processed_bar_count, signal_count, first_reward, second_reward, reward FROM single_day_run").fetchone()
    assert tuple(daily_statistics) == (3, 0, None, None, None)
